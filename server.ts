...
const web3 = new Web3(
    ...
    new Web3.providers.WebsocketProvider(
        'wss://mainnet.infura.io/ws/v3/1b21c8ff6c3f4c72bc607321fabebdc1'
    )
);

function getTransactionObj (tx: any) {
    if (!tx.raw) return
    try {
...
async function getAccount(addressStr: any) {
    let res = await axios.get(`http://localhost:9001/account/${addressStr}`)
    return res.data.account
}

function stringToHex(str: string) {
    let hexValue = '0x' + parseInt(str, 10).toString(16)
    return hexValue
}
...
let methods = {
    ...
    eth_blockNumber: async function (args: any, callback: any) {
        let result = '0x0'
        try {
            let res = await axios.get(`http://localhost:9001/sync-newest-cycle`)
    ...
    eth_getBalance: async function (args: any, callback: any) {
        let balance = '0x0'
        try {
            console.log('Getting eth_getBalance')
            let address = args[0]
    ...
    eth_sendTransaction: async function (args: any, callback: any) {
        ...
        console.log("Running eth_sendTransaction")
        callback(null, result);
    },
    eth_sendRawTransaction: async function (args: any, callback: any) {
        ...
        await axios.post(`http://localhost:9001/inject`, tx)
    ...
    eth_getTransactionReceipt: async function (args: any, callback: any) {
        ...
        let txHash = args[0]
        let res = await axios.get(`http://localhost:9001/tx/${txHash}`)
